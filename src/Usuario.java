...
// O codigo estÃƒÂ¡ bem seco, mas jÃƒÂ¡ podemos iniciar o modelo de como o usuario irÃƒÂ¡ se cadastrar para o MIDAS //
...
public class Usuario extends JFrame {
	...
	public Usuario(){
		...
		setLayout(new FlowLayout()); // Configura o layout do frame (ainda nÃƒÂ£o descobri como melhorar)
		...
		senha.setToolTipText("A senha deve ter tamanho 6 e conter characteres e numeros"); // refazer com o pessoal de seguranÃƒÂ§a
		add(senha);
		senhaUsuario = senha.getText();
		
		
		campoSenha = new JTextField(" ConfirmaÃ§Ã£o de Senha:",12);
		add(campoSenha);
		campoSenha.setEditable(false);
		...
		box_profissao.setMaximumRowCount(5); // Numero mÃƒÂ¡ximo de itens
		add(box_profissao);
...
//		Manipulador de evento de mouse
	...
	public class ButtonHandler implements ActionListener{ // classe interna que indica a aÃƒÂ§ÃƒÂ£o de manipulaÃƒÂ§ÃƒÂ£o
		
		public void actionPerformed(ActionEvent event){
		
			if(event.getSource() == enviar){
				Usuario.this.dispose();
				JOptionPane.showMessageDialog(null, "Cadastro Enviado com sucesso", "ConfirmaÃƒÂ§ÃƒÂ£o de Cadastro", JOptionPane.OK_OPTION);
			}
		}
	}
